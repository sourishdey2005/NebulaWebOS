...
import { AppDefinition, StickyNote } from '../types';
...
export const Desktop: React.FC<DesktopProps> = ({ apps, onOpenApp, stickyNotes = [], setStickyNotes }) => {
  ...
  return (
    <div className="absolute inset-0 p-6 flex flex-col flex-wrap content-start items-start gap-y-4 gap-x-4 w-full h-[calc(100%-48px)]">
      {/* Sticky Notes Layer */}
      {stickyNotes.map((note) => (
        <div 
            key={note.id}
            className="absolute p-4 rounded-lg shadow-lg w-48 min-h-[160px] flex flex-col animate-in zoom-in duration-200"
            style={{ 
                left: note.x, 
                top: note.y, 
      ...
      {/* App Icons Layer */}
      <div className="flex flex-col flex-wrap content-start items-start gap-4 h-full relative z-10 pointer-events-none">
          {apps.map((app) => (
            <button
              key={app.id}
              onClick={() => onOpenApp(app.id)}
              className="pointer-events-auto group flex flex-col items-center justify-center gap-2 w-24 p-2 rounded-xl hover:bg-white/5 transition-colors focus:bg-white/10 focus:outline-none ring-0 outline-none"
            >
